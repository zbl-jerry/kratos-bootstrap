...
func LoadBootstrapConfig(configPath string) error {
	...
	if hotReloadEnabled {
		...
		// 启动配置监听器
		go func() {
			err := cfg.Watch("", func(key string, value config.Value) {
				log.Infof("Configuration changed: %s", key)
				reloadConfigurations(cfg)
			})
			if err != nil {
				log.Errorf("Failed to watch configuration: %v", err)
			}
		}()
		
		log.Info("Configuration hot reload enabled")
	}
